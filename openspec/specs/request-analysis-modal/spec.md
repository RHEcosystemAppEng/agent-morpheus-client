...
- **WHEN** the user selects Single Repository mode and fills in a valid CVE ID, Source Repository, and Commit ID in the request analysis modal
- **AND** optionally toggles Private repository and provides credentials
- **AND** clicks the "Submit Analysis Request" button
- **THEN** the modal validates the CVE ID format and required fields (Source Repository, Commit ID)
...
- **AND** the form validation passes for the selected mode (SBOM: CVE ID and file; Single Repository: CVE ID, Source Repository, Commit ID)
