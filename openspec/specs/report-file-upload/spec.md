...
- **WHEN** a user submits a multipart form to `/api/v1/sbom-reports/upload-cyclonedx` with a file that is not valid JSON
...
- **WHEN** a user submits a multipart form to `/api/v1/sbom-reports/upload-cyclonedx` with a valid JSON file that does not contain the `metadata.component.name` field
- **THEN** the system returns HTTP 400 (Bad Request)
- **AND** the response body is a JSON object mapping field names to error messages
...
- **WHEN** a user submits a multipart form to `/api/v1/sbom-reports/upload-cyclonedx` without providing a CVE ID
- **THEN** the system returns HTTP 400 (Bad Request)
- **AND** the response body is a JSON object mapping field names to error messages
...
- **WHEN** a user submits a multipart form to `/api/v1/sbom-reports/upload-cyclonedx` with a CVE ID that does not match the official CVE regex pattern `^CVE-[0-9]{4}-[0-9]{4,19}$`
- **THEN** the system returns HTTP 400 (Bad Request)
- **AND** the response body is a JSON object mapping field names to error messages
...
- **WHEN** a user submits a multipart form to `/api/v1/sbom-reports/upload-cyclonedx` without providing a file
- **THEN** the system returns HTTP 400 (Bad Request)
- **AND** the response body is a JSON object mapping field names to error messages
...
- **WHEN** a user submits a multipart form to `/api/v1/sbom-reports/upload-cyclonedx` with both an invalid CVE ID and an invalid file
- **THEN** the system returns HTTP 400 (Bad Request)
- **AND** the response body is a JSON object mapping field names to error messages
