...
Frontend MUST use React 18+ with functional components and hooks. UI components MUST use PatternFly 6 design system for consistency and accessibility. Build tooling MUST use Vite for fast development and optimized production builds. All UI components MUST be responsive and accessible. Each React component MUST NOT exceed 50 lines of code. If a component exceeds this limit, it MUST be refactored into smaller, focused sub-components. Native HTML tags like `div`, `p`, and `span` should be avoided when possible in favor of patternfly components. Usually text can be directly embedded in the components instead of wrapped in html tags.
